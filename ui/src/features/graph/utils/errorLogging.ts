 ...
 * @module features/graph/utils/errorLogging
 */

import { GraphError, type GraphErrorCode } from './errorHandler';
...
/**
 * Log a graph error with full context
 *
...
export function logGraphError(
  error: GraphError | Error | unknown,
  context: ErrorContext = {}
): void {
  const graphError = normalizeError(error);
  const entry = createLogEntry(graphError, context);

  // Console logging with appropriate level
  if (graphError.retryable) {
    console.warn('[Graph Error]', entry);
  } else {
    console.error('[Graph Error]', entry);
  }

  // In development, log additional details
  if (import.meta.env.DEV) {
    console.group('Error Details');
    console.log('Code:', graphError.code);
    console.log('Message:', graphError.message);
    console.log('Context:', context);
    if (graphError.details) {
      console.log('Details:', graphError.details);
    }
    if (graphError.cause) {
      console.log('Cause:', graphError.cause);
    }
    console.groupEnd();
  }
...
/**
 * Normalize any error to a GraphError
 */
function normalizeError(error: unknown): GraphError {
 ...
 * trackErrorMetrics(graphError, { operation: 'fetchGraph' });
 *
 * // Later, get metrics
...
export function trackErrorMetrics(
  ...
  const graphError = error instanceof GraphError
    ? error
    : new GraphError(error.message, 'UNKNOWN_ERROR');

  // Increment total
  metrics.total++;

  // Track by code
  metrics.byCode[graphError.code] = (metrics.byCode[graphError.code] ?? 0) + 1;
 ...
 */
export function getErrorRate(code: GraphErrorCode | string): number {
 ...
 */
export function formatErrorForDebug(error: GraphError | Error): string {
  if (error instanceof GraphError) {
    const parts = [
...
if (import.meta.env.DEV && typeof window !== 'undefined') {
  (window as { __graphErrorDebug?: unknown }).__graphErrorDebug = {
