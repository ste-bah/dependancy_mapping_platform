#------------------------------------------------------------------------------
# Code Reviewer - Development Environment
...
locals {
  ...
  # Development defaults - cost-optimized, single NAT
...
module "secrets" {
  ...
  environment             = var.environment
  recovery_window_in_days = 0 # Force delete in dev for faster iteration
...
module "vpc" {
  ...
  single_nat_gateway = local.single_nat_gateway # Single NAT for dev cost savings
...
# Note: Log group ARN created by ECS module, using placeholder pattern
#------------------------------------------------------------------------------
...
module "rds" {
  ...
  # Development sizing - smaller instances
  ...
  backup_retention_period = 1 # Minimal backup in dev

  # Protection settings - disabled in dev
  deletion_protection = false

  # Monitoring
  create_cloudwatch_alarms = false # Disabled in dev
...
module "elasticache" {
  ...
  # Backup configuration
  snapshot_retention_limit = 0 # No snapshots in dev
...
module "ecs" {
  ...
  desired_count          = 1    # Single task in dev
  enable_execute_command = true # Enable debugging

  # Auto scaling - minimal in dev
  min_capacity              = 1
  max_capacity              = 2
  ...
  app_url        = var.app_url
  api_url        = var.api_url
  enable_swagger = true # Enable API docs in dev
  enable_metrics = true
...
module "cloudwatch" {
  ...
  # Dashboard - enabled in dev for visibility
